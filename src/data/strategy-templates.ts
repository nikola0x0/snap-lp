...
function generateSpotDistribution(centerBin: number, range: number): Array<{binId: number; weight: number}> {
  const bins = []
...
function generateCurveDistribution(centerBin: number, range: number): Array<{binId: number; weight: number}> {
  const bins = []
  let totalWeight = 0
...
function generateBidAskDistribution(centerBin: number, range: number): Array<{binId: number; weight: number}> {
  const bins = []
...
export const STRATEGY_TEMPLATES: StrategyTemplate[] = [
  {
    ...
    description: 'Wide range strategy for stable pairs like USDC-USDT with minimal IL risk',
  ...
  
  {
    ...
    description: 'Even liquidity distribution around current price for moderate volatility pairs',
    riskLevel: 'Balanced',
    riskRating: 3,
    estimatedAPR: 15,
  ...
  
  {
    ...
    description: 'Tight range around current price for maximum capital efficiency and fees',
  ...
  
  {
    id: 'swing-trader',
    name: 'Swing Trader',
    description: 'Bid-ask distribution to profit from price swings and volatility',
    riskLevel: 'Aggressive',
    riskRating: 4,
    estimatedAPR: 28,
  ...
  
  {
    ...
    description: 'Ultra-tight range strategy for quick profits on stable pairs. Perfect for experienced traders.',
    riskLevel: 'Aggressive',
    riskRating: 5,
    estimatedAPR: 45,
    impermanentLossRisk: 5,
  ...
  
  {
    ...
    description: 'Long-term accumulation strategy using wide ranges. Popularized by crypto whales.',
    riskLevel: 'Conservative',
    riskRating: 2,
    estimatedAPR: 12,
    impermanentLossRisk: 2,
...
export function adjustTemplateForPool(template: StrategyTemplate, activeBin: number): StrategyTemplate {
  const adjustedBinDistribution = template.binConfiguration.binDistribution.map(bin => ({
    ...bin,
...
export function getTemplatesByCategory(category: 'Conservative' | 'Balanced' | 'Aggressive') {
  return STRATEGY_TEMPLATES.filter(template => template.riskLevel === category)
}
