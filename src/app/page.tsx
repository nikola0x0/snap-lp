...
export type DashboardSection = 'pools' | 'templates' | 'simulator' | 'deploy' | 'portfolio' | 'create'
...
export default function Home() {
    ...
    switch (currentStep) {
      ...
      case 'create':
        return <div className="p-8 text-center text-muted-foreground">
          <strong>Strategy Builder</strong> - Create and monetize your own strategies (Coming Soon)
        </div>
      default:
        return <PoolsSection />
    }
  ...
  const getCurrentStepNumber = () => {
    const stepMap = { pools: 1, templates: 2, simulator: 3, deploy: 4, portfolio: 5, create: 6 }
        ...
        {(isPoolSelected() || isTemplateSelected()) && (
          <div className="p-4 border-b bg-muted/30">
            <div className="space-y-2">
              <div className="text-xs text-muted-foreground uppercase tracking-wide">Current Selection</div>
              {isPoolSelected() && (
                <div className="flex items-center gap-2">
                  <CheckCircle className="w-3 h-3 text-green-600" />
                  <span className="text-xs font-medium">{getTokenPairSymbol()}</span>
                </div>
              )}
              {isTemplateSelected() && (
                <div className="flex items-center gap-2">
