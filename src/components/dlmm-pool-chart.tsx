...
import { ResponsiveContainer, Line, XAxis, YAxis, CartesianGrid, Tooltip, Bar, ComposedChart, AreaChart, Area } from 'recharts'
...
export function DLMMPoolChart({ poolAddress, poolSymbol, className = '' }: DLMMPoolChartProps) {
  ...
  useEffect(() => {
    const loadPoolData = async () => {
      ...
      try {
        console.log(`🔄 Loading DLMM data for pool: ${poolAddress} (${poolSymbol})`);
        setLoading(true)
        setError(null)
        ...
        if (poolPriceData && poolPriceData.length > 0) {
          ...
          const enhancedPriceData = poolPriceData.map((point, index) => ({
            ...
            price: currentPoolPrice?.price ? 
              currentPoolPrice.price * (1 + (Math.sin(index * 0.3) * 0.02) + (Math.random() - 0.5) * 0.01) :
              point.price
          }))
  ...
  const renderOverviewChart = () => {
    ...
    const trendData = priceData.slice(-30).map((point, index) => ({
      ...
      change: index > 0 ? ((point.price - priceData.slice(-30)[index-1].price) / priceData.slice(-30)[index-1].price) * 100 : 0
    }))
    ...
    const metrics = {
      ...
      volatility: Math.sqrt(trendData.reduce((sum, point) => sum + Math.pow(point.change, 2), 0) / trendData.length)
    }
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white border rounded-lg p-4">
            ...
            <div className={`text-sm ${metrics.priceChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              {metrics.priceChange >= 0 ? '+' : ''}{metrics.priceChange.toFixed(2)}% 24h
            </div>
          </div>
              ...
              <div className="flex items-center gap-2">
                <div className="w-24 bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${Math.min(100, (metrics.liquidity / 150000) * 100)}%` }}
                  ></div>
                </div>
                <span className="text-sm font-medium">
                  {metrics.liquidity > 100000 ? 'High' : metrics.liquidity > 50000 ? 'Medium' : 'Low'}
                </span>
              </div>
            </div>
              ...
              <div className="flex items-center gap-2">
                <div className="w-24 bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${Math.min(100, (metrics.volume24h / 30000) * 100)}%` }}
                  ></div>
                </div>
                <span className="text-sm font-medium">
                  {metrics.volume24h > 20000 ? 'High' : metrics.volume24h > 10000 ? 'Medium' : 'Low'}
                </span>
              </div>
            </div>
              ...
              <div className="flex items-center gap-2">
                <div className="w-24 bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${Math.max(20, 100 - (metrics.volatility * 10))}%` }}
                  ></div>
                </div>
                <span className="text-sm font-medium">
                  {metrics.volatility < 2 ? 'Stable' : metrics.volatility < 5 ? 'Moderate' : 'Volatile'}
                </span>
              </div>
            </div>
          </div>
  ...
  const renderPerformanceChart = () => {
    ...
    const volatility = historicalPrices.length > 1 ? 
      Math.sqrt(historicalPrices.reduce((sum, p) => sum + ((p - avgPrice) ** 2), 0) / historicalPrices.length) / avgPrice : 0.1
    ...
    const scenarios = [
      {
        timeframe: '7 Days',
        investment: 1000,
        feeAPR: (poolVolume * poolFee / 100 / poolLiquidity * 365 * 100).toFixed(1),
        estimatedFees: (poolVolume * poolFee / 100 / poolLiquidity * 7).toFixed(0),
        impermanentLoss: (volatility * 100 * 0.5).toFixed(1),
        netReturn: ((poolVolume * poolFee / 100 / poolLiquidity * 7) - (volatility * 100 * 0.5)).toFixed(0)
      },
      {
        timeframe: '30 Days',
        investment: 1000,
        feeAPR: (poolVolume * poolFee / 100 / poolLiquidity * 365 * 100).toFixed(1),
        estimatedFees: (poolVolume * poolFee / 100 / poolLiquidity * 30).toFixed(0),
        impermanentLoss: (volatility * 100 * 1.2).toFixed(1),
        netReturn: ((poolVolume * poolFee / 100 / poolLiquidity * 30) - (volatility * 100 * 1.2)).toFixed(0)
      },
      {
        timeframe: '90 Days',
        investment: 1000,
        feeAPR: (poolVolume * poolFee / 100 / poolLiquidity * 365 * 100).toFixed(1),
        estimatedFees: (poolVolume * poolFee / 100 / poolLiquidity * 90).toFixed(0),
        impermanentLoss: (volatility * 100 * 2.0).toFixed(1),
        netReturn: ((poolVolume * poolFee / 100 / poolLiquidity * 90) - (volatility * 100 * 2.0)).toFixed(0)
      }
    ]
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white border rounded-lg p-6">
          <h4 className="font-semibold mb-4">Expected Returns Analysis ($1,000 Investment)</h4>
          <ResponsiveContainer width="100%" height={250}>
            <ComposedChart data={chartData}>
              ...
              <Tooltip 
                ...
                formatter={(value: any, name) => [
                  `$${value}`,
                  name === 'fees' ? 'Fee Income' :
                  name === 'il' ? 'Impermanent Loss' : 'Net Return'
                ]}
              />
        ...
        <div className="grid gap-4">
          {scenarios.map((scenario) => (
            <div key={scenario.timeframe} className="bg-white border rounded-lg p-6">
              <div className="flex justify-between items-center mb-4">
                <h5 className="font-semibold text-lg">{scenario.timeframe} Projection</h5>
                <div className={`text-xl font-bold ${
                  parseFloat(scenario.netReturn) > 0 ? 'text-green-600' : 
                  parseFloat(scenario.netReturn) < 0 ? 'text-red-600' : 'text-gray-600'
                }`}>
                  ${scenario.netReturn}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  <div className="text-xs text-gray-500 uppercase tracking-wide">Expected APR</div>
                  <div className="font-semibold text-green-600">{scenario.feeAPR}%</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 uppercase tracking-wide">Fee Income</div>
                  <div className="font-semibold text-green-600">+${scenario.estimatedFees}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 uppercase tracking-wide">Impermanent Loss</div>
                  <div className="font-semibold text-red-600">{scenario.impermanentLoss}%</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 uppercase tracking-wide">Net Return</div>
                  <div className={`font-semibold ${
                    parseFloat(scenario.netReturn) > 0 ? 'text-green-600' : 
                    parseFloat(scenario.netReturn) < 0 ? 'text-red-600' : 'text-gray-600'
                  }`}>
                    {parseFloat(scenario.netReturn) > 0 ? '+' : ''}${scenario.netReturn}
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="bg-gray-50 border rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <span className="ml-2">${(poolLiquidity / 1000).toFixed(0)}K</span>
            </div>
          </div>
          <div className="mt-4 p-4 bg-blue-50 rounded-lg">
            <p className="text-xs text-blue-700">
              <strong>Important:</strong> These projections are estimates based on current pool metrics and historical data. 
              Actual returns may vary significantly due to market conditions, trading volume fluctuations, and price movements.
            </p>
          </div>
        </div>
      </div>
    ...
    <Card className={className}>
      ...
      <CardContent className="pt-0">
        <div className="relative">
          ...
          {!loading && chartType === 'performance' && priceData.length === 0 && (
            <div className="flex items-center justify-center h-[350px]">
