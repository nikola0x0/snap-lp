...
export function TradingChart({ mint, symbol, className = '', height = 400 }: TradingChartProps) {
  ...
  useEffect(() => {
    const findPool = async () => {
      try {
        ...
        console.log(`Available pools:`, pools.pools.map(p => ({ 
          address: p.address, 
          tokenX: p.tokenX, 
          tokenY: p.tokenY 
        })));
        ...
        let matchingPool = pools.pools.find(p => 
          p.tokenX === mint || p.tokenY === mint
        );
        
        // If no exact match, use a consistent fallback based on symbol
        if (!matchingPool) {
          console.log(`No exact match for ${mint}, using fallback logic for ${symbol}`);
          
          // Use consistent pool selection based on symbol
          if (symbol.toUpperCase().includes('SOL')) {
            matchingPool = pools.pools.find(p => p.address.includes('sol-usdc')) || pools.pools[0];
  ...
  useEffect(() => {
    const loadDLMMData = async () => {
      ...
      try {
        console.log(`Loading DLMM data for pool ${poolAddress}, timeRange: ${timeRange}`);
        setLoading(true)
        setError(null)
        ...
        setError(err instanceof Error ? err.message : 'Failed to load DLMM data')
      } finally {
        setLoading(false)
      }
    }
          ...
          {loading && (
            <div className="absolute inset-0 flex items-center justify-center bg-white/80 z-10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">Loading DLMM pool data...</span>
              </div>
            </div>
          )}
              ...
              <LineChart data={chartData}>
                ...
                <Tooltip 
                  ...
                  formatter={(value: any, name: string) => {
                    if (name === 'price') return [formatPrice(value), 'DLMM Pool Price']
