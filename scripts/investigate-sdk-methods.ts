...
async function investigateSDKMethods() {
  ...
  try {
    ...
    console.log(
      "getQuote function:",
      dlmm.getQuote.toString().substring(0, 200) + "...",
    );
    ...
    console.log(
      "getBinsReserveInformation method:",
      typeof dlmm.getBinsReserveInformation,
    );

    try {
      const result3 = await dlmm.getBinsReserveInformation(
        new PublicKey(poolAddress),
      );
      console.log(
        "✅ getBinsReserveInformation with 1 param succeeded:",
        typeof result3,
      );
    } catch (error) {
      console.log(
        "❌ getBinsReserveInformation with 1 param failed:",
        error.message,
      );
    }
    ...
    console.log(
      "getMaxAmountOutWithFee method:",
      typeof dlmm.getMaxAmountOutWithFee,
    );

    try {
      const result4 = await dlmm.getMaxAmountOutWithFee(
        new PublicKey(poolAddress),
        1000000,
      );
      console.log(
        "✅ getMaxAmountOutWithFee with 2 params succeeded:",
        typeof result4,
      );
    } catch (error) {
      console.log(
        "❌ getMaxAmountOutWithFee with 2 params failed:",
        error.message,
      );
    }
  } catch (error) {
