  ...
  try {
    ...
    for (const poolAddress of allPoolAddresses) {
      try {
        ...
        if (metadata.baseMint === WSOL_MINT || metadata.quoteMint === WSOL_MINT) {
          wsolPools.push({
            address: poolAddress,
        ...
        console.error(`❌ Error fetching ${poolAddress}:`, error instanceof Error ? error.message : error);
      }
    }
    ...
    wsolPools.slice(0, 5).forEach((pool, index) => {
      ...
      const otherToken = isWSOLBase ? pool.quoteMint.slice(0, 8) : pool.baseMint.slice(0, 8);

      console.log(`  {`);
      console.log(`    address: "${pool.address}",`);
      console.log(`    name: "${isWSOLBase ? 'WSOL' : otherToken}/${isWSOLBase ? otherToken : 'WSOL'}",`);
