...
async function findTopPools() {
  try {
    ...
    sortedByVolume.forEach((pool, index) => {
      ...
      console.log(
        `${index + 1}. ${pair.padEnd(20)} | Vol: $${volume.padEnd(15)} | Liq: $${liquidity}`,
      );
    ...
    sortedByLiquidity.forEach((pool, index) => {
      ...
      console.log(
        `${index + 1}. ${pair.padEnd(20)} | Liq: $${liquidity.padEnd(15)} | Vol: $${volume}`,
      );
      console.log(`   Address: ${pool.address}`);
      console.log();
    });
    ...
    topPools.forEach((pool, index) => {
      ...
      console.log(
        `    description: "${pool.baseToken?.symbol} / ${pool.quoteToken?.symbol} - High volume pair",`,
      );
