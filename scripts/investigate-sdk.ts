...
async function investigatePoolDetails(dlmm: LiquidityBookServices, poolAddresses: string[]) {
  ...
  for (let i = 0; i < Math.min(3, poolAddresses.length); i++) {
    ...
    try {
      ...
      // Test getPairAccount
      try {
        const pairAccount = await dlmm.getPairAccount(new PublicKey(poolAddress));
        ...
        if (pairAccount && 'activeId' in pairAccount && 'binStep' in pairAccount) {
...
async function investigateAdvanced() {
  ...
  const availableMethods = Object.getOwnPropertyNames(Object.getPrototypeOf(dlmm))
    .filter(name => typeof (dlmm as any)[name] === 'function' && name !== 'constructor');
